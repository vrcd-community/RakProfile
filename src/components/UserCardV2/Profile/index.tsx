...
export default function ClientCard({isSelf, isAdmin, uid}: UserCardV2Props) {
  const {loading, claims, error} = useProfile(isSelf ? "me" : uid);
  ...
  return (
    <Card className="w-full">
      ...
      <CardContent className="grid gap-6">
        <Bio edit={isSelf || isAdmin} bio={claims?.bio!} claims={claims}/>
      </CardContent>
      <Separator/>
      <CardContent>
